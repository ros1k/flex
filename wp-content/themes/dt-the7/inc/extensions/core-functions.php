...
function dt_core_send_mail() {
	...
	if ( ! empty( $fields ) && ! $errors ) {
		...
		$custom_error_msg = of_get_option( 'custom_success_messages' );
		// message
		if ( $send ) {
			$errors = empty( $custom_error_msg ) ? _x( 'Your message has been sent.', 'feedback msg', 'the7mk2' ) : $custom_error_msg;
		} else {
			$errors = empty( $custom_error_msg ) ? _x( 'The message has not been sent. Plaese try again.', 'feedback msg', 'the7mk2' ) : $custom_error_msg;
		}
	}
